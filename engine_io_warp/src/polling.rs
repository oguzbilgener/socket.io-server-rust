...
use engine_io_server::transport::{
    ...
    RequestResult, ResponseBodyData, TransportBase, TransportEvent,
};
...
#[async_trait]
impl PollingTransport<AdapterResponse, Bytes> for WarpPollingTransport {
    fn new(
    ...
    async fn handle_request(
        ...
        match (request_context.http_method, self.mode) {
            ...
            (HttpMethod::Post, _) => self.handle_data_request(request_context, body).await,
...
impl WarpPollingTransport {
    ...
    async fn handle_data_request(
        ...
        packets.into_iter().for_each(|packet| {
            ...
            let _ = self.event_sender.send(TransportEvent::Packet { packet });
        });
    ...
    fn handle_options_request(&self) -> RequestResult<AdapterResponse> {
        Ok(Response::builder()
            .status(200)
    ...
    fn respond_with_status_code_only(&self, status_code: u16) -> RequestResult<AdapterResponse> {
        Ok(Response::builder()
            .status(status_code)
            .body(Body::empty())
            .unwrap()
...
fn respond(
    ...
    let common_headers = get_common_polling_response_headers(context, mode == Mode::Xhr);
